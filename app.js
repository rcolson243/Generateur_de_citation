...
let btn_nouveau =  document.querySelector("#nouveau");
...
btn_nouveau.addEventListener('click', () => {
    do {
        nb_aleatoire = genererNombreEntier(citations.length)

    } while (nb_aleatoire == dernier);
    citation.textContent = citations[nb_aleatoire][0];
    auteur.textContent    = citations[nb_aleatoire][1];
    dernier               = nb_aleatoire;
});
